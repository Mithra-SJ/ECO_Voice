...
exports.ModelMonitor = exports.hasVisualAd = exports.DEFAULT_MONITOR_SUMMARY_INTERVAL_MS = void 0;
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const linux_impulse_runner_types_1 = require("../library/classifier/linux-impulse-runner-types");
const tsee_1 = require("tsee");
exports.DEFAULT_MONITOR_SUMMARY_INTERVAL_MS = 60000;
const MONITOR_PREFIX = '\x1b[34m[MON]\x1b[0m';
const DEFAULT_THRESHOLD = 0.5;
function checkFileExists(file) {
    return new Promise(resolve => {
        return promises_1.default.access(file, promises_1.default.constants.F_OK)
            .then(() => resolve(true))
            .catch(() => resolve(false));
    });
}
...
*/
function hasVisualAd(model) {
    return [
        linux_impulse_runner_types_1.RunnerHelloHasAnomaly.VisualGMM,
        linux_impulse_runner_types_1.RunnerHelloHasAnomaly.VisualPatchcore,
        linux_impulse_runner_types_1.RunnerHelloHasAnomaly.VisualCustom
    ].includes(model.modelParameters.has_anomaly);
}
exports.hasVisualAd = hasVisualAd;
class StorageManager {
    ...
    static async getStorageManager(conf, model) {
    ...
    constructor(conf, model) {
        ...
        this._firstIndex = Number.MAX_SAFE_INTEGER;
        // this limits the number of records in a single directory
        this._storageIndexSegment = 1000;
        this._storageSizeMaxBytes = 250 * 1024 * 1024;
        this._storageSize = 0;
        this._config = conf;
        this._model = model;
    }
    async getHighestAndLowestIndexes(path) {
        let highest = 0;
        let lowest = Number.MAX_SAFE_INTEGER;
        // get a list of all json files in the storage directory
        let files = await promises_1.default.readdir(path);
        // filter out non-json files
        files = files.filter(file => file.endsWith('.json'));
        // filter out files that don't have a number as the name
        files = files.filter(file => !isNaN(Number(path_1.default.basename(file, '.json'))));
        // find the highest and lowest indexes
        for (const file of files) {
            const index = Number(path_1.default.basename(file, '.json'));
            if (index > highest) {
                highest = index;
            }
            if (index < lowest) {
    ...
    async getStorageSize(path = this._storagePath) {
        const dirSize = async (dir) => {
            ...
            const sizesPromiseArr = files.map(async (file) => {
                ...
                if (file.isFile())
                    return (await promises_1.default.stat(filePath)).size;
    ...
    async initialize() {
        ...
        const expectedStoragePath = path_1.default.join(this._config.getDefaultStoragePath(), modelProjectId.toString(), deploymentVersion?.toString() ?? '');
        ...
        this._metricsDir = path_1.default.join(this._storagePath, 'metrics');
        if (await checkFileExists(this._metricsDir) === false) {
            await promises_1.default.mkdir(this._metricsDir, { recursive: true });
        }
        ...
        if (await checkFileExists(this._storagePath) === false) {
            await promises_1.default.mkdir(this._storagePath, { recursive: true });
        }
        else {
            // if storage exist, then try to restore first and last index
            // get list of all directories (segments) in the storage directory
            let dirs = await promises_1.default.readdir(this._storagePath, { withFileTypes: true });
            ...
            if (dirs.length === 0) {
                ...
                ({ highest } = await this.getHighestAndLowestIndexes(path_1.default.join(this._storagePath, highestSegment.toString())));
                ({ lowest } = await this.getHighestAndLowestIndexes(path_1.default.join(this._storagePath, lowestSegment.toString())));
            }
    ...
    async cleanupStorage() {
        ...
        let lowestSegmentPath = path_1.default.join(this._storagePath, dirs[0].name);
        ...
        const stats = await promises_1.default.stat(lowestSegmentPath);
        if (stats.isDirectory()) {
            // In Node.js v16, fs.rmdir with recursive option is deprecated
            // fs.rm is the recommended replacement
            await promises_1.default.rm(lowestSegmentPath, { recursive: true, force: true });
        }
        else {
            await promises_1.default.unlink(lowestSegmentPath);
        }
    ...
    async saveInferenceMetrics(metricsStr, fileName) {
        const metricsPath = path_1.default.join(this._metricsDir, `${fileName}.json`);
        await promises_1.default.writeFile(metricsPath, metricsStr);
    }
    async getLatestInferenceMetrics() {
        try {
            const files = await promises_1.default.readdir(this._metricsDir);
            ...
            return await promises_1.default.readFile(path_1.default.join(this._metricsDir, latestFile), 'utf-8');
        }
        catch (error) {
    ...
    async saveRecord(record) {
        ...
        const recordPath = path_1.default.join(await this.getPathByIndex(record.index), record.index.toString() + '.json');
        if (record.rawData.type === 'jpg') {
            const imgPath = path_1.default.join(await this.getPathByIndex(record.index), record.index.toString() + '.jpg');
            const buf = Buffer.from(record.rawData.bufferBase64, 'base64');
            await promises_1.default.writeFile(imgPath, buf);
            rawDataSize = buf.length;
            record.rawData.bufferBase64 = imgPath;
        }
        const recordStr = JSON.stringify(record, null, 4);
        await promises_1.default.writeFile(recordPath, recordStr, 'utf-8');
    ...
    async getRecord(index) {
        const recordPath = path_1.default.join(await this.getPathByIndex(index), index.toString() + '.json');
        if (await checkFileExists(recordPath) === false) {
            return undefined;
        }
        let record = JSON.parse(await promises_1.default.readFile(recordPath, 'utf-8'));
        if (record.rawData.type === 'jpg') {
            let fileBuf;
            ...
            fileBuf = await promises_1.default.readFile(record.rawData.bufferBase64);
...
class MetricsCalculator {
    ...
    static async getMetricsInstance(config, storage, model) {
        ...
        if (!summaryWindowMs || isNaN(summaryWindowMs) || summaryWindowMs <= 0) {
            summaryWindowMs = exports.DEFAULT_MONITOR_SUMMARY_INTERVAL_MS;
        }
    ...
    constructor(storage, model, summaryWindowMs) {
        this._confidenceThresholds = new Map();
    ...
    async updateMetrics(record) {
        ...
        const incrementClassificationCounter = (key) => {
    ...
    async saveMetrics() {
        ...
        await this._storageManager.saveInferenceMetrics(JSON.stringify(cache, null, 2), `${this._metrics.start.index}_${this._metrics.end.index}`);
    }
 ...
 */
class ModelMonitor extends tsee_1.EventEmitter {
    constructor(_config) {
        super();
     ...
     */
    static async getModelMonitor(config, model) {
    ...
    async processResult(result, rawData) {
        let record = {
...
//# sourceMappingURL=model-monitor.js.map