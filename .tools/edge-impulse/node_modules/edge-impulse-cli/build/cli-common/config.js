...
const api_1 = require("../sdk/studio/api");
...
class Config {
     ...
     */
    static async writeFileAtomic(path, content) {
        const tempPath = path + '.' + crypto_1.default.randomBytes(8).toString('hex') + '.lock';
        try {
            ...
            if (typeof content === 'string') {
                ...
                await fs_1.default.promises.writeFile(tempPath, content);
            }
            // rename should be atomic
            await fs_1.default.promises.rename(tempPath, path);
        }
        finally {
    ...
    constructor() {
        ...
        this._filename = path_1.default.join(os_1.default.homedir(), 'edge-impulse-config.json');
    }
    /**
    ...
    async clean() {
        if (await Config.exists(this._filename)) {
            await util_1.default.promisify(fs_1.default.unlink)(this._filename);
        }
    }
    ...
    async setUploaderProjectId(projectId) {
        let config = await this.load();
    ...
    async setLinuxProjectId(projectId) {
        let config = await this.load();
        config.linuxProjectId = projectId;
        await this.store(config);
    }
    async getApiKeyForProject(projectId) {
        let config = await this.load();
        if (!config) {
            return undefined;
        }
    ...
    async setApiKeyForProject(projectId, apiKey) {
        let config = await this.load();
     ...
     */
    async verifyLogin(devMode, apiKey, verifyType = 'project') {
        let config = await this.load();
        ...
        if (!config.host) {
            if (devMode) {
                const hostRes = await inquirer_1.default.prompt([{
                        type: 'list',
                        choices: [
        ...
        if (host === 'localhost' && get_ips_1.ips.length === 0) {
        ...
        this._api = new api_1.EdgeImpulseApi({
    ...
    async getDataForwarderDevice(deviceId) {
        let config = await this.load();
        return config.dataForwarderDevices[deviceId];
    }
    async storeDataForwarderDevice(deviceId, data) {
        let config = await this.load();
        config.dataForwarderDevices[deviceId] = data;
        await this.store(config);
    }
    async deleteDataForwarderDevice(deviceId) {
        let config = await this.load();
        delete config.dataForwarderDevices[deviceId];
        await this.store(config);
    }
    async getDaemonDevice(deviceId) {
        let config = await this.load();
        return config.daemonDevices[deviceId];
    }
    async storeDaemonDevice(deviceId, data) {
        let config = await this.load();
        config.daemonDevices[deviceId] = data;
        await this.store(config);
    }
    async deleteDaemonDevice(deviceId) {
        let config = await this.load();
    ...
    async setStorageMaxSizeMb(maxSizeMb) {
        let config = await this.load();
    ...
    async storeBlockId(blockId) {
        let config = await this.load();
        config.runner.blockId = blockId;
        await this.store(config);
    }
    async setRunnerImpulseIdForProjectId(projectId, impulseId) {
        let config = await this.load();
    ...
    async getRunnerImpulseIdForProjectId(projectId) {
        let config = await this.load();
        if (!config.runner.impulseIdsForProjectId) {
            return null;
        }
        let ret = config.runner.impulseIdsForProjectId[projectId.toString()];
        return ret ? ret.impulseId : null;
    }
    async setRunnerModelVariantForProjectId(projectId, variant) {
        let config = await this.load();
    ...
    async getRunnerModelVariantForProjectId(projectId) {
        let config = await this.load();
        if (!config.runner.modelVariantsForProjectId) {
            return null;
        }
        let ret = config.runner.modelVariantsForProjectId[projectId.toString()];
    ...
    async setDeploymentVersion(deploymentVersion) {
        let config = await this.load();
        config.runner.deploymentVersion = deploymentVersion;
        await this.store(config);
    }
    async getStudioUrl(whitelabelId) {
    ...
    getDefaultModelsPath() {
        return path_1.default.join(this.getDefaultRunnerPath(), 'models');
    }
    getDefaultStoragePath() {
        return path_1.default.join(this.getDefaultRunnerPath(), 'storage');
    }
    getDefaultRunnerPath() {
        return path_1.default.join(os_1.default.homedir(), '.ei-linux-runner');
    }
    async load() {
        if (!await Config.exists(this._filename)) {
            return {
        ...
        try {
            let c = JSON.parse(await util_1.default.promisify(fs_1.default.readFile)(this._filename, 'utf-8'));
     ...
     * @param verifyType
     */
    async verifyAccess(config, verifyType) {
        const api = this._api;
        if (!api) {
    ...
    async getJWTToken(api, config, host) {
        const username = await inquirer_1.default.prompt({
            type: 'input',
            name: 'username',
            message: `What is your user name or e-mail address (${host})?`
        });
        const { needPassword, email, whitelabels } = await api.user.getUserNeedToSetPassword(username.username);
        if (needPassword) {
        ...
        try {
            ...
            if ((ex.message || ex.toString()).startsWith('ERR_TOTP_TOKEN IS REQUIRED')) {
                ...
                while (1) {
                    try {
                        const totpToken = await inquirer_1.default.prompt({
                            type: 'input',
                            name: 'totpToken',
    ...
    async setMonitorSummaryIntervalMs(ms) {
        let config = await this.load();
