...
exports.checkXmlMatchesAnyFormat = exports.parseXmlLabelsFile = exports.xmlToJson = void 0;
const label_file_types_1 = require("./label-file-types");
const parse_label_json_1 = require("./parse-label-json");
/**
 ...
 */
function parseXmlLabelsFile(xmlDoc, format) {
    const xmlFormat = label_file_types_1.supportedXmlLabelFormats.find(f => f.info.key === format);
    ...
    // Validate the JSON
    const isValid = (0, parse_label_json_1.validateJsonSchema)(xmlFormat.fn.schema, json);
    if (!isValid.valid) {
    ...
    try {
        ...
        return {
            success: false,
            reason: `Error parsing type '${xmlFormat.info.name}'; ${ex.message}`
        };
    }
}
exports.parseXmlLabelsFile = parseXmlLabelsFile;
/**
 ...
 */
function checkXmlMatchesAnyFormat(xmlFile, formatsToTry) {
    let jsonObj;
    try {
        // Convert to JSON
    ...
    const allFormats = formatsToTry ? formatsToTry : label_file_types_1.supportedXmlLabelFormats;
    for (const format of allFormats) {
        if (format.fn.fileType !== 'xml') {
            continue;
        }
        const formatMatch = (0, parse_label_json_1.checkJsonMatchesFormat)(format, jsonObj);
...
exports.checkXmlMatchesAnyFormat = checkXmlMatchesAnyFormat;
//# sourceMappingURL=parse-label-xml.js.map