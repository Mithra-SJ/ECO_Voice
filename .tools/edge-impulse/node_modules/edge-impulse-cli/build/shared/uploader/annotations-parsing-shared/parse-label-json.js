...
exports.parseTxtLabelsFile = exports.parseJsonLabelsFile = exports.checkJsonMatchesAnyFormat = exports.validateJsonSchema = exports.checkJsonMatchesFormat = void 0;
const label_file_types_1 = require("./label-file-types");
const label_files_shared_1 = require("./label-files-shared");
/**
 ...
 */
function checkJsonMatchesFormat(format, json) {
    const valOpts = {
...
exports.checkJsonMatchesFormat = checkJsonMatchesFormat;
/**
 ...
 */
function validateJsonSchema(schema, instance, opts) {
    const validateObject = (io, constraint, scope) => {
        if (!constraint) {
            return {
                valid: true
            };
        ...
        if (constraint.type === 'object' && !constraint.isMap) {
            ...
            for (const [key, value] of Object.entries(constraint.value || [])) {
                ...
                if (entry) {
                    const entryIsValid = validateObject(entry[1], value, [...scope, key]);
            ...
            const valuesConstraint = constraint.values.type === 'recursive' ? {
                type: 'either',
            ...
            for (let idx = 0; idx < rootArr.length; idx++) {
                const entryIsValid = validateObject(rootArr[idx], (typeof constraint.values === 'function') ? constraint.values(rootArr[idx]) : constraint.values, [...scope, idx]);
                if (!entryIsValid.valid) {
                    return entryIsValid;
                }
            ...
            if (constraint.valid) {
                const validEntries = constraint.valid.map(cv => JSON.stringify(cv));
            ...
            if (constraint.validationFn) {
                ...
                if (constraint.type === 'string') {
                    validationRes = constraint.validationFn(io);
                }
 ...
 */
function checkJsonMatchesAnyFormat(jsonFile, formatsToTry) {
    const json = JSON.parse(jsonFile);
    const allFormats = formatsToTry ? formatsToTry : label_file_types_1.supportedJsonLabelFormats;
...
exports.checkJsonMatchesAnyFormat = checkJsonMatchesAnyFormat;
/**
 ...
 */
function parseJsonLabelsFile(jsonFile, format) {
    const json = JSON.parse(jsonFile);
    const jsonFormat = label_file_types_1.supportedJsonLabelFormats.find(f => f.info.key === format);
    if (!jsonFormat) {
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: false,
            reason: `Error parsing type '${jsonFormat.info.name}'; ${ex.message}`
        };
    }
}
exports.parseJsonLabelsFile = parseJsonLabelsFile;
/**
 ...
 */
function parseTxtLabelsFile(file, filename, format) {
    const labelFormat = label_file_types_1.supportedTxtLabelFormats.find(f => f.info.key === format);
    ...
    try {
        ...
        let labels = {};
        labels[(0, label_files_shared_1.removeExtension)(filename)] = labelsRes;
        return {
            success: true,
        ...
        return {
            success: false,
            reason: `Error parsing type '${labelFormat.info.name}'; ${ex.message}`
        };
    }
}
exports.parseTxtLabelsFile = parseTxtLabelsFile;
//# sourceMappingURL=parse-label-json.js.map