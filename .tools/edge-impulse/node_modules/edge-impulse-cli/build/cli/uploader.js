...
const make_image_1 = require("../cli-common/make-image");
const init_cli_app_1 = require("../cli-common/init-cli-app");
const bounding_box_file_types_1 = require("../shared/bounding-box-file-types");
const fs_helpers_1 = require("../cli-common/fs-helpers");
const dataset_converter_cli_1 = require("./dataset-converter-cli");
const label_file_types_1 = require("../shared/uploader/annotations-parsing-shared/label-file-types");
...
const logAllAnnotationFormats = () => {
    [{
        ...
        console.log((0, label_file_types_1.listAllAnnotationFormats)(type.key)
...
(async () => {
    ...
    const init = await (0, init_cli_app_1.initCliApp)(cliOptions);
    ...
    try {
        ...
        if (infoFileArgv) {
            try {
                let infoFile = (0, bounding_box_file_types_1.parseUploaderInfo)(await fs_1.default.promises.readFile(infoFileArgv, 'utf-8'));
                files = [];
                for (let f of infoFile.files) {
                    if (!path_1.default.isAbsolute(f.path)) {
                        f.path = path_1.default.join(path_1.default.dirname(infoFileArgv), f.path);
                    }
                    files.push({
                        category: f.category,
                        name: f.name,
            ...
            if (!fs_1.default.statSync(directoryArgv).isDirectory()) {
                console.log(directoryArgv + ' is not a directory');
                process.exit(1);
            }
            const allFiles = (0, dataset_converter_cli_1.getAllFilesInFolder)(directoryArgv);
            ...
            if (annotationFormatArgv) {
                ...
                datasetFormat = (0, label_file_types_1.getMetadataForFormat)(annotationFormatArgv);
                if (!datasetFormat) {
                ...
                const formatMatches = datasetFormat.formatStyle !== 'txt' ?
                    (0, dataset_converter_cli_1.checkDatasetMatchesFormat)(allFiles, datasetFormat.key) : true;
                if (!formatMatches) {
                ...
                const derivedFormat = (0, dataset_converter_cli_1.deriveDatasetFormat)(allFiles);
                if (derivedFormat) {
                    datasetFormat = (0, label_file_types_1.getMetadataForFormat)(derivedFormat);
                }
            }
            if (datasetFormat) {
                // Convert the directory into EI dataset
                try {
                    const datasetConverter = new dataset_converter_cli_1.DatasetConverterHelperCli(datasetFormat, { silent: silentArgv, validExtensions: make_image_1.VALID_EXTENSIONS });
                    ...
                    for (const sample of samples) {
                        ...
                        files.push({
                            path: path_1.default.join(sample.directory, sample.filename),
                            name: undefined,
                ...
                const dir = await fs_1.default.promises.realpath(directoryArgv);
                let dirs = await fs_1.default.promises.readdir(dir);
                ...
                for (let c of ['training', 'testing']) {
                    for (let f of await fs_1.default.promises.readdir(path_1.default.join(dir, c))) {
                        let fullPath = path_1.default.join(dir, c, f);
                        if (f.startsWith('.'))
                            continue;
                        files.push({
                            category: c,
            ...
            if (fileArgs.length === 1 && path_1.default.basename(fileArgs[0]) === 'bounding_boxes.labels') {
                console.log(``);
            ...
            fileArgs = fileArgs.filter(f => path_1.default.basename(f) !== 'bounding_boxes.labels');
            if (!fileArgs[0]) {
                console.log('Requires at least one argument (a ' +
                    make_image_1.VALID_EXTENSIONS.slice(0, make_image_1.VALID_EXTENSIONS.length - 1).join(', ') + ' or ' +
                    make_image_1.VALID_EXTENSIONS[make_image_1.VALID_EXTENSIONS.length - 1] + ' file)');
                process.exit(1);
            }
            let metadata;
            if (metadataArgv) {
                try {
                    metadata = JSON.parse(metadataArgv);
                }
                catch (ex) {
            ...
            if (openmvArgv) {
                ...
                if (!fs_1.default.statSync(fileArgs[0]).isDirectory()) {
                    console.log(fileArgs[0] + ' is not a directory (required for --format-openmv)');
                    process.exit(1);
                }
                let categoryFolders = fs_1.default.readdirSync(fileArgs[0]).filter(d => d.endsWith('.class'));
                ...
                for (let categoryFolder of categoryFolders) {
                    for (let f of fs_1.default.readdirSync(path_1.default.join(fileArgs[0], categoryFolder))) {
                        files.push({
                            path: path_1.default.join(fileArgs[0], categoryFolder, f),
                            name: undefined,
                            category: 'split',
                ...
                if (fileArgs.length === 1 && fs_1.default.statSync(fileArgs[0]).isDirectory()) {
                    console.log('Cannot handle file, file is a directory. Please use --directory to upload.');
                    process.exit(1);
                }
                // Check extension is valid
                if (make_image_1.VALID_EXTENSIONS.indexOf(path_1.default.extname(fileArgs[0].toLowerCase())) === -1) {
                    console.log('Cannot handle this file, only ' +
                        make_image_1.VALID_EXTENSIONS.join(', ') + ' supported:', fileArgs[0]);
                    process.exit(1);
                }
                // Windows doesn't do expansion like Mac and Linux...
                if (path_1.default.basename(fileArgs[0], path_1.default.extname(fileArgs[0])) === '*') {
                    fileArgs = (await util_1.default.promisify(fs_1.default.readdir)(path_1.default.dirname(fileArgs[0]))).filter(v => {
                        return path_1.default.extname(v) === path_1.default.extname(fileArgs[0]);
                    }).map(f => path_1.default.join(path_1.default.dirname(fileArgs[0]), f));
                }
        ...
        const { projectId, devKeys } = await (0, init_cli_app_1.setupCliApp)(configFactory, config, cliOptions, undefined);
        ...
        if (!silentArgv) {
            ...
            if (projectInfo.project.whitelabelId) {
                const whitelabelRes = await config.api.whitelabels.getWhitelabelDomain(projectInfo.project.whitelabelId);
        ...
        let allDirectories = [...new Set(files.map(f => path_1.default.resolve(path_1.default.dirname(f.path))))];
        const loadBoundingBoxCache = async (directory) => {
            let labelsFile = path_1.default.join(directory, 'bounding_boxes.labels');
            if (!await fs_helpers_1.FSHelpers.exists(labelsFile)) {
                boundingBoxCache[directory] = undefined;
            }
            else {
                try {
                    boundingBoxCache[directory] = (0, bounding_box_file_types_1.parseBoundingBoxLabels)(await fs_1.default.promises.readFile(labelsFile, 'utf-8'));
                }
                catch (ex2) {
                    let ex = ex2;
        ...
        await (0, tiny_async_pool_1.default)(10, allDirectories, loadBoundingBoxCache);
        const processFile = async (file) => {
            const boundingBoxesFile = boundingBoxCache[path_1.default.resolve(path_1.default.dirname(file.path))];
            let boundingBoxes = boundingBoxesFile ?
                (boundingBoxesFile.boundingBoxes[path_1.default.basename(file.path)] || undefined) :
                undefined;
            ...
            try {
                ...
                if (file.name) {
                    filename = file.name;
                    let ext = path_1.default.extname(file.path);
                    if (ext) {
                        filename += ext;
                    }
                }
                ...
                await (0, make_image_1.upload)({
                    filename: filename,
                    buffer: await fs_1.default.promises.readFile(file.path),
                ...
                if (!progressIvArgv) {
                    console.log(`[${ixS}/${totalFilesLength}] Uploading`, file.path, 'OK (' + (Date.now() - hrstart) + ' ms)');
                }
                else if (ix === 1) {
                ...
                let ix = ++fileIx;
                let ixS = ix.toString().padStart(totalFilesLength.toString().length, ' ');
                console.log(`[${ixS}/${totalFilesLength}] Failed to upload`, file.path, ex.message || ex.toString());
                failed++;
            }
        };
        let progressIv;
        ...
        await (0, tiny_async_pool_1.default)(concurrency, files, processFile);
        if (progressIv) {
