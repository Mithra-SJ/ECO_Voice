...
const can_flash_serial_1 = require("./can-flash-serial");
const jpeg_js_1 = __importDefault(require("jpeg-js"));
const remote_mgmt_service_1 = require("../cli-common/remote-mgmt-service");
...
const cliOptions = {
    ...
    getProjectFromConfig: async (deviceId) => {
        if (!deviceId)
            return undefined;
        return await configFactory.getDaemonDevice(deviceId);
    }
};
class SerialDevice extends tsee_1.EventEmitter {
    constructor(config, projectId, serialConnector, serialProtocol, deviceConfig) {
            ...
            try {
                if (Date.now() - +this._lastSnapshot > 200 &&
                    ...
                    if (depth === 1 || depth === 3) {
                        ...
                        let jpegImageData = jpeg_js_1.default.encode({
                            data: frameData,
                            width: width,
                            height: height,
                        }, 80);
    ...
    async sampleRequest(data, ee) {
        ...
        if (s.sensor?.startsWith('Camera (')) {
            ...
            function waitForSamplingDone(ee2) {
                    ...
                    if (deviceResponse.file &&
                        ...
                        const response = await (0, undici_1.fetch)(url, {
                            ...
                            headers: {
                                ...
                                'x-file-name': (0, encoding_1.default)(deviceResponse.filename),
                                'x-label': (0, encoding_1.default)(dr.label),
...
(async () => {
    try {
        ...
        const initRes = await (0, init_cli_app_1.initCliApp)(cliOptions);
...
async function connectToSerial(eiConfig, deviceId, baudRate, clean) {
    // if this is set it means we have a connection
    let config;
    let remoteMgmt;
    serial = new serial_connector_1.SerialConnector(deviceId, baudRate, cliOptions.verboseArgv);
    ...
    async function connectLogic() {
        ...
        try {
            ...
            const { projectId, devKeys } = await (0, init_cli_app_1.setupCliApp)(configFactory, eiConfig, cliOptions, serialId);
            if (serialId) {
            ...
            if (!remoteMgmt) {
                ...
                remoteMgmt = new remote_mgmt_service_1.RemoteMgmt(projectId, devKeys, Object.assign({
                    command: 'edge-impulse-daemon'
                }, eiConfig), device, undefined, // model monitor object
                // model monitor object
                url => new ws_1.default(url), async (currName) => {
            ...
            if (await (0, can_flash_serial_1.canFlashSerial)(deviceId)) {
                // flashed...
...
async function setupWizard(eiConfig, serialProtocol, deviceConfig, devKeys) {
    if (!serial) {
    ...
    try {
        ...
        if (!deviceConfig.wifi.connected && credentials.askWifi !== false && deviceConfig.wifi.present &&
            !setupWizardRan) {
            let inqSetup = await inquirer_1.default.prompt([{
                    type: 'confirm',
            ...
            if (inqSetup.setupWifi) {
                ...
                process.stdout.write(' OK\n');
                let inqWifi = await inquirer_1.default.prompt([{
                        type: 'search-list',
                ...
                let network = inqWifi.wifi;
                let pass = '';
                if (network.security !== ei_serial_protocol_1.EiSerialWifiSecurity.EI_SECURITY_NONE) {
                    let inqPass = await inquirer_1.default.prompt([{
                            type: 'input',
