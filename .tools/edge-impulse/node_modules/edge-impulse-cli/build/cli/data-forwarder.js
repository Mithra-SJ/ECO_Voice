...
(async () => {
    try {
        ...
        console.log('');
        let serialPath = await (0, find_serial_1.findSerial)(whichDeviceArgv);
...
async function connectToSerial(eiConfig, serialPath, baudRate, clean) {
    // if this is set we have a connection to the server
    let ws;
    let dataForwarderConfig;
    const serial = new serial_connector_1.SerialConnector(serialPath, baudRate);
    serial.on('error', err => {
    ...
    async function sendHello() {
        ...
        ws.once('message', async (helloResponse) => {
            let ret = cbor_1.default.decode(helloResponse);
            if (!ret.hello) {
                ...
                console.log(TCP_PREFIX, `Go to ${studioUrl}/studio/${dataForwarderConfig.projectId}/acquisition/training ` +
                    `to build your machine learning model!`);
            }
        });
        ws.send(cbor_1.default.encode(req));
    }
    ...
    function attachWsHandlers() {
        ...
        ws.on('message', async (data) => {
            ...
            let d = cbor_1.default.decode(data);
            // hello messages are handled in sendHello()
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            if (typeof d.hello !== 'undefined')
                return;
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            if (typeof d.sample !== 'undefined') {
                ...
                try {
                    if (!dataForwarderConfig) {
                        ...
                        if (ws) {
                            ws.send(cbor_1.default.encode(res4));
                        }
                        return;
                    }
                    let res = {
                    ...
                    if (ws) {
                        ws.send(cbor_1.default.encode(res));
                    }
                    ...
                    if (ws) {
                        ws.send(cbor_1.default.encode(res2));
                    }
                    console.log(SERIAL_PREFIX, 'Reading data from device...');
                    let allDataBuffers = [];
                    const onData = (db) => allDataBuffers.push(db);
                    ...
                    if (!values.every(v => {
                        ...
                        console.log(SERIAL_PREFIX, 'Invalid data collected. All values should be numeric, and every line should have ' +
                            dataForwarderConfig.sensors.length + ' values.', values);
                    ...
                    let hmac = node_crypto_1.default.createHmac('sha256', dataForwarderConfig.hmacKey);
                    hmac.update(encoded);
                    ...
                    if (ws) {
                        ws.send(cbor_1.default.encode(res5));
                    }
                    const response = await (0, undici_1.fetch)(eiConfig.endpoints.internal.ingestion + s.path, {
                        method: 'POST',
                        headers: {
                            'x-api-key': dataForwarderConfig.apiKey,
                            'x-file-name': (0, encoding_1.default)(s.label + '.json'),
                            'x-label': (0, encoding_1.default)(s.label),
                    ...
                    if (ws) {
                        ws.send(cbor_1.default.encode(res3));
                    }
        ...
        ws.on('error', err => {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            console.error(TCP_PREFIX, `Error connecting to ${eiConfig.endpoints.internal.ws}`, err.code || err);
        });
        ws.on('close', () => {
    ...
    async function ws_connect() {
        ...
        try {
            ...
            ws = new ws_1.default(eiConfig.endpoints.internal.ws);
            attachWsHandlers();
        }
        catch (ex) {
...
async function getAndConfigureProject(eiConfig, serial) {
    const { projectId, devKeys } = await (0, init_cli_app_1.setupCliApp)(configFactory, eiConfig, cliOptions, (await serial.getMACAddress()) || undefined);
    ...
    while (axes.split(',').filter(f => !!f.trim()).length !== sensorInfo.sensorCount) {
        axes = (await inquirer_1.default.prompt([{
                type: 'input',
...
async function checkName(eiConfig, projectId, deviceId) {
    try {
        ...
        if (nameDevice.nameDevice !== currName) {
            (await eiConfig.api.devices.renameDevice(projectId, deviceId, { name: nameDevice.nameDevice }));
        }
...
async function getProjectName(eiConfig, projectId) {
    try {
...
async function getSensorInfo(serial, desiredFrequency) {
    let dataBuffers = [];
    const onData = (b) => {
        dataBuffers.push({
            ts: Date.now(),
