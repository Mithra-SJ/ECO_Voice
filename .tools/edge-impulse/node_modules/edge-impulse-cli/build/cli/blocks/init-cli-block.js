...
exports.InitCLIBlock = void 0;
const inquirer_1 = __importDefault(require("inquirer"));
const path_1 = __importDefault(require("path"));
const blocks_helper_1 = require("./blocks-helper");
const fs_1 = __importDefault(require("fs"));
class InitCLIBlock {
    constructor(config, folder, blockConfigManager, opts) {
        this._config = config;
        this._folder = folder;
        this._paths = {
            eiBlockConfig: path_1.default.join(folder, '.ei-block-config'),
            parametersJson: path_1.default.join(folder, 'parameters.json'),
        };
    ...
    async getOrganization() {
        ...
        if (!organizations.organizations || organizations.organizations.length === 0) {
            ...
            let orgInqRes = await inquirer_1.default.prompt([{
                    type: 'search-list',
    ...
    async initBlock(type, organizationInfo) {
        switch (type) {
            case 'dsp':
    ...
    async initDSPBlock(organizationInfo) {
        const organizationId = organizationInfo.organization.id;
        const api = this._config.api;
        let parameters;
        if (await (0, blocks_helper_1.pathExists)(this._paths.parametersJson)) {
            parameters = JSON.parse(await fs_1.default.promises.readFile(this._paths.parametersJson, 'utf-8'));
    ...
    async initDeployBlock(organizationInfo) {
        const organizationId = organizationInfo.organization.id;
        const api = this._config.api;
        ...
        let params;
        if (await (0, blocks_helper_1.pathExists)(this._paths.parametersJson)) {
            params = JSON.parse(await fs_1.default.promises.readFile(this._paths.parametersJson, 'utf-8'));
            if (!params.version) {
                params.version = 1;
            }
            params.type = 'deploy';
        ...
        if (params?.info?.name) {
            ...
            blockName = (await inquirer_1.default.prompt([{
                    type: 'input',
                    name: 'name',
        ...
        if (params?.info?.description) {
            ...
            blockDescription = (await inquirer_1.default.prompt([{
                    type: 'input',
                    name: 'description',
        ...
        if (params?.info?.category) {
            ...
            category = (await inquirer_1.default.prompt([{
                    type: 'list',
                    name: 'category',
                    choices: [
                        {
        ...
        if (params?.info?.integrateUrl) {
            ...
            integrateUrl = (await inquirer_1.default.prompt([{
                    type: 'input',
                    name: 'integrateUrl',
        ...
        if (params?.info?.cliArguments) {
            ...
            cliArguments = (await inquirer_1.default.prompt([{
                    type: 'input',
                    name: 'cliArguments',
        ...
        if (typeof params?.info?.mountLearnBlock === 'boolean') {
            ...
            mountLearnBlock = resp.mountLearnBlock;
        }
        let supportsEonCompiler;
        if (typeof params?.info?.supportsEonCompiler === 'boolean') {
            ...
            supportsEonCompiler = resp.supportsEonCompiler;
        }
        let showOptimizations;
        if (typeof params?.info?.showOptimizations === 'boolean') {
            ...
            showOptimizations = resp.showOptimizations;
        }
        let privileged;
    ...
    async initMachineLearningBlock(organizationInfo) {
        const organizationId = organizationInfo.organization.id;
        const api = this._config.api;
        let existingBlocks = (await api.organizationBlocks.listOrganizationTransferLearningBlocks(organizationId)).transferLearningBlocks;
        let createOrUpdateInqRes = await this.createOrUpdate(existingBlocks);
        let selectedBlock;
        if (createOrUpdateInqRes === 'update') {
            selectedBlock = await this.selectExistingBlock(existingBlocks);
        ...
        const showVisualAnomalyOption = organizationInfo.experiments?.find(x => x.type === 'custom_visual_ad_blocks')?.enabled;
        ...
        if (params?.info?.operatesOn) {
            ...
            blockTlOperatesOn = (await inquirer_1.default.prompt([{
                    type: 'list',
                    name: 'operatesOn',
        ...
        if (blockTlOperatesOn === 'image' || blockTlOperatesOn === 'object_detection') {
            if (params?.info?.imageInputScaling) {
                ...
                blockTlImageInputScaling = (await inquirer_1.default.prompt([{
                        type: 'list',
                        name: 'inputScaling',
        ...
        let blockTlObjectDetectionLastLayer;
        if (blockTlOperatesOn === 'object_detection') {
            if (params?.info?.objectDetectionLastLayer) {
                ...
                blockTlObjectDetectionLastLayer = (await inquirer_1.default.prompt([{
                        type: 'list',
                        name: 'lastLayer',
        ...
        if (typeof params?.info?.indRequiresGpu === 'boolean') {
            ...
            let blockTlCanRunWhere = (await inquirer_1.default.prompt([{
                    type: 'list',
                    name: 'canRunWhere',
                    default: 'cpu-or-gpu',
                    choices: [
        ...
        if (params?.info?.repositoryUrl) {
            ...
            repoUrl = await (0, blocks_helper_1.guessRepoUrl)();
        }
        let customModelVariants;
    ...
    async initSyntheticDataBlock(organizationInfo) {
        const organizationId = organizationInfo.organization.id;
        const api = this._config.api;
        let existingBlocks = (await api.organizationBlocks.listOrganizationTransformationBlocks(organizationId)).transformationBlocks
    ...
    async initAIActionBlock(organizationInfo) {
        const organizationId = organizationInfo.organization.id;
        const api = this._config.api;
        let existingBlocks = (await api.organizationBlocks.listOrganizationTransformationBlocks(organizationId)).transformationBlocks
        ...
        if (params?.info?.operatesOn) {
            ...
            operatesOn = (await inquirer_1.default.prompt([{
                    type: 'checkbox',
                    name: 'operatesOn',
    ...
    async initTransformBlock(organizationInfo) {
        const organizationId = organizationInfo.organization.id;
        const api = this._config.api;
        let existingBlocks = (await api.organizationBlocks.listOrganizationTransformationBlocks(organizationId)).transformationBlocks
        ...
        if (params?.info?.operatesOn) {
            ...
            blockOperatesOn = (await inquirer_1.default.prompt([{
                    type: 'list',
                    name: 'operatesOn',
                    choices: [
                        {
        ...
        if (params?.info?.transformMountpoints) {
            ...
            if (buckets.buckets && buckets.buckets.length > 0) {
                transformMountpoints = (await inquirer_1.default.prompt([{
                        type: 'checkbox',
                        name: 'buckets',
        ...
        let showInCreateTransformationJob;
    ...
    async createOrUpdate(existingBlocks) {
        if (this._opts.alwaysCreateNewBlock) {
            return 'create';
        }
        return existingBlocks.length > 0 ? (await inquirer_1.default.prompt([{
                type: 'list',
                choices: [
                    {
    ...
    async selectExistingBlock(existingBlocks) {
        // Update an existing block
        // Choose a block ID
        let blockChoiceInqRes = await inquirer_1.default.prompt([{
                type: 'list',
...
exports.InitCLIBlock = InitCLIBlock;
//# sourceMappingURL=init-cli-block.js.map